...
const getMeDataSuccess = ( user: IUser, token: string ) => ({
...
export const getMeData = () => (dispatch: any) => {
    ...
    get( `${config.apiBase}user/`, { token: storrage.read('token') } )
        .then( ({ user, token }: any) => {
            dispatch(getMeDataSuccess(user, token));
        } )
        .catch( ( errors: IError ) => {
...
const registrationSuccess = ( user: IUser, token: string ) => ({
...
export const registration = ( data: IUser ) => (dispatch: any) => {
    ...
    post( `${config.apiBase}user`, data, { token: storrage.read('token') } )
        .then( ({ user, token }: any) => {
